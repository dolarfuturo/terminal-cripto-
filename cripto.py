...
assets_config = {
    "BTC/USDT": {"sym": "BTC-USD", "fallback": 82632},
    "ETH/USDT": {"sym": "ETH-USD", "fallback": 2500},
    "BNB/USDT": {"sym": "BNB-USD", "fallback": 600},
    "SOL/USDT": {"sym": "SOL-USD", "fallback": 150},
    "XRP/USDT": {"sym": "XRP-USD", "fallback": 0.60},
    "DOGE/USDT": {"sym": "DOGE-USD", "fallback": 0.15}
}

if 'market_states' not in st.session_state:
    st.session_state.market_states = {}
    for label, cfg in assets_config.items():
        val = get_midpoint_v13(cfg["sym"], cfg["fallback"])
    ...
    try:
        ...
        with placeholder.container():
            ...
            for label, state in st.session_state.market_states.items():
                price = yf.Ticker(assets_config[label]["sym"]).fast_info['last_price']
                ...
                <div class="row-container">
                    ...
                    <div class="w-col"><div>{price:,.2f}</div><div style="color:{cor_v}; font-size:11px;">{seta_v} {var_reset:+.2f}%</div></div>
            ...
            st.markdown(f"""<div class="footer"><div><span class="dot"></span> LIVESTREAM ATIVO</div><div>LONDRES: {now_lon.strftime('%H:%M:%S')}</div><div>BRASÍLIA: {now_br.strftime('%H:%M:%S')}</div><div>NEW YORK: {now_ny.strftime('%H:%M:%S')}</div></div>""", unsafe_allow_html=True)
