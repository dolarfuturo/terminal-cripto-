...
st.markdown("""
    ...
    .row-container { display: flex; width: 100%; align-items: center; padding: 20px 0; border-bottom: 1px solid #151515; justify-content: space-between; }
...
assets = {
    "BTC/USDT": {"tk": "BTC-USD", "fb": 82632},
    "ETH/USDT": {"tk": "ETH-USD", "fb": 2500},
    "BNB/USDT": {"tk": "BNB-USD", "fb": 600}
}

if 'states' not in st.session_state:
    st.session_state.states = {}
    for label, cfg in assets.items():
        val = get_midpoint_v13(cfg["tk"], cfg["fb"])
...
while True:
    try:
        ...
        # Auto-Reset Binance (18:00 BR / 00:00 UTC)
        ...
        with placeholder.container():
            ...
            for label, state in st.session_state.states.items():
                price = yf.Ticker(assets[label]["tk"]).fast_info['last_price']
                ...
                fundo_decisao = "background: rgba(255, 255, 0, 0.3);" if 0.59 <= abs(var) <= 0.65 else ""
                ...
                <div style="display: flex; justify-content: center; gap: 80px; margin-top: 5px; margin-bottom: 10px; border-bottom: 1px dotted #333;">
                    <div style="text-align: center;">
                        <span style="color: #888; font-size: 9px; text-transform: uppercase;">ResetVision: </span>
                        <span style="color: #ffffff; font-size: 14px; font-weight: bold;">{int(rv):,}</span>
                    </div>
                    <div style="text-align: center;">
                        <span style="color: #888; font-size: 9px; text-transform: uppercase;">ÂncoraVision: </span>
                        <span style="color: #00e6ff; font-size: 14px; font-weight: bold;">{int(mp):,}</span>
                    </div>
            ...
            st.markdown(f"""
                <div class="footer">
                    ...
                    <div>BRASÍLIA: {datetime.now(br_tz).strftime('%H:%M:%S')}</div>
                    <div>NY: {datetime.now(ny_tz).strftime('%H:%M:%S')}</div>
                    <div>LONDRES: {datetime.now(lon_tz).strftime('%H:%M:%S')}</div>
                </div>""", unsafe_allow_html=True)
        time.sleep(1)
