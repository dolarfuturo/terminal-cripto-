...
# 1. SETUP ALPHA - VISUAL REFINADO (ORIGINAL)
...
# 2. MOTOR DE CÁLCULO (ADAPTADO PARA MULTI-TICKER)
...
while True:
    try:
        ...
        if now_br.hour == 18 and now_br.minute == 0 and now_br.second < 2:
            for label, cfg in assets.items():
                novo_valor = get_midpoint_v13(cfg["sym"], cfg["fb"])
                st.session_state.states[label]["mp"] = novo_valor
                st.session_state.states[label]["rv"] = novo_valor
            st.rerun()
        ...
        with placeholder.container():
            ...
            for label, state in st.session_state.states.items():
                ticker = yf.Ticker(assets[label]["sym"])
                price = ticker.fast_info['last_price']
                mp, rv = state["mp"], state["rv"]
                
                var = ((price / mp) - 1) * 100
                if var >= 1.35: st.session_state.states[label]["mp"] = int(mp * 1.0122)
                elif var <= -1.35: st.session_state.states[label]["mp"] = int(mp * 0.9878)
                ...
                st.markdown(f"""
                    <div class="row-container">
                        ...
                        <div class="w-col">
                            <div style="font-weight: bold; line-height: 1.1;">{int(price) if price > 10 else f"{price:.4f}"}</div>
                            <div style="color:{cor_v}; font-size:11px; font-weight:bold;">{seta_v} {var_reset:+.2f}%</div>
                        </div>
                        <div class="w-col" style="{estilo_ex_t}">{int(mp*1.0122) if mp > 10 else f"{mp*1.0122:.4f}"}</div>
                        <div class="w-col" style="color:#FFA500;">{int(mp*1.0083) if mp > 10 else f"{mp*1.0083:.4f}"}</div>
                        <div class="w-col" style="{fundo_decisao}">{int(mp*1.0061) if mp > 10 else f"{mp*1.0061:.4f}"}</div>
                        <div class="w-col" style="color:#00CED1;">{int(mp*1.0040) if mp > 10 else f"{mp*1.0040:.4f}"}</div>
                        <div class="w-col" style="color:#FFA500;">{int(mp*0.9939) if mp > 10 else f"{mp*0.9939:.4f}"}</div>
                        <div class="w-col" style="{estilo_ex_f}">{int(mp*0.9878) if mp > 10 else f"{mp*0.9878:.4f}"}</div>
                    </div>
                """, unsafe_allow_html=True)

            # RODAPÉ E LEGENDAS ORIGINAIS (MANTIDAS)
            st.markdown(f"""
                <div style="display: flex; justify-content: center; gap: 80px; margin-top: 15px; padding-bottom: 80px;">
                    <div style="text-align: center;">
                        ...
                        <div style="color: #ffffff; font-size: 19px; font-weight: bold;">INFO TERMINAL</div>
                    </div>
                    <div style="text-align: center;">
                        <div style="color: #888; font-size: 10px; text-transform: uppercase;">ÂncoraVision (Móvel)</div>
                        <div style="color: #00e6ff; font-size: 19px; font-weight: bold;">ESCADA ATIVA</div>
                    </div>
                </div>
                <div class="footer">
