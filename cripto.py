...
st.markdown("""
    ...
    .title-gold { color: #D4AF37; font-size: 26px; font-weight: 900; text-align: center; padding: 20px; letter-spacing: 3px; }
    
    /* Grid de Monitoramento Midpoint */
    .header-container { display: flex; width: 100%; padding: 15px 0; border-bottom: 2px solid #D4AF37; background: #050505; justify-content: space-between; }
    .h-col { font-size: 11px; color: #777; text-transform: uppercase; text-align: center; font-weight: 800; flex: 1; letter-spacing: 1px; }
    
    .row-container { display: flex; width: 100%; align-items: center; padding: 35px 0; border-bottom: 1px solid #111; justify-content: space-between; }
    .w-col { flex: 1; text-align: center; font-family: 'monospace'; font-size: 21px; font-weight: 800; color: #EEE; white-space: nowrap; }
    
    /* Footer Stream */
    .footer { position: fixed; bottom: 0; left: 0; width: 100%; background: #000; color: #FFF; text-align: center; padding: 15px; font-size: 13px; border-top: 1px solid #222; display: flex; justify-content: center; align-items: center; gap: 40px; z-index: 1000; }
    ...
    .reset-alert { background-color: #D4AF37; color: #000; text-align: center; font-weight: 900; padding: 12px; font-size: 14px; position: fixed; top: 0; width: 100%; z-index: 2000; }
    </style>
    """, unsafe_allow_html=True)

# 2. MOTOR DE CALIBRAGEM (MIDPOINT LOGIC)
def get_midpoint_institutional():
    try:
        br_tz = pytz.timezone('America/Sao_Paulo')
        now_br = datetime.now(br_tz)
        
        # Travamento FDS (Conforme dados TradingView fornecidos)
        if now_br.weekday() >= 5 or (now_br.weekday() == 0 and now_br.hour < 18):
            return 89792.500
        
        # Auto-recalc pós 18:00 BR
...
while True:
    try:
        ...
        # Reset automático no fechamento da janela
        ...
        with placeholder.container():
            if now_br.hour == 18 and now_br.minute == 0:
                st.markdown('<div class="reset-alert">⚠️ ALERTA: MIDPOINT RECALIBRADO COM BASE NA JANELA 11:30-18:00</div>', unsafe_allow_html=True)

            st.markdown(f"""
                <div class="header-container">
                    <div class="h-col">ASSET</div><div class="h-col">LAST PRICE (3D)</div>
